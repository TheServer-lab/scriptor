# scriptor.py
# Scriptor v0.1 — Plugin-ready editor with multi-language highlighting and polished UI.
...
class PluginAPI:
    ...
    def register_hook(self, name, func):
        """Register a callable for a named hook (on_open, on_save, on_event)."""
...
class Hooks:
    ...
    def on_open(self, path):
        self.pm.call_hook("on_open", path)

    def on_save(self, path):
        self.pm.call_hook("on_save", path)

    def on_event(self, name, **kwargs):
        self.pm.call_hook("on_event", name=name, **kwargs)
...
class MultiLangHighlighter(QSyntaxHighlighter):
    ...
    def highlightBlock(self, text):
        ...
        for pattern, fmt in self.rules:
            flags = re.DOTALL if "{" not in pattern else 0
...
class CodeEditor(QPlainTextEdit):
    def __init__(self, filename=None):
        ...
        lang = MultiLangHighlighter.language_for_filename(self.file_path.name) if self.file_path else "python"
...
class Ribbon(QWidget):
    ...
    def set_theme(self, dark: bool):
        if dark:
            self.setStyleSheet(
                "QWidget#ribbon { background: #2b2b2d; border-bottom: 1px solid #3c3c3c; } QToolButton { color: #e6eef6; } QToolButton:hover { background: #333539; }"
            )
            sep_color = "#3c3c3c"
            btn_text = "#e6eef6"
        else:
            self.setStyleSheet(
                "QWidget#ribbon { background: #f4f4f4; border-bottom: 1px solid #d0d0d0; } QToolButton { color: #222222; } QToolButton:hover { background: #e8e8e8; }"
            )
            sep_color = "#d0d0d0"
            btn_text = "#222222"
...
class Scriptor(QMainWindow):
    ...
    def _create_actions_and_menus(self):
        ...
        close_act.triggered.connect(self.on_close_current_tab)
        ...
        plugin_install_act = QAction("Install Plugin...", self)
        plugin_install_act.triggered.connect(self.install_plugin)
        plugin_reload_act = QAction("Reload Plugins", self)
        plugin_reload_act.triggered.connect(self.reload_plugins)
        plugin_uninstall_act = QAction("Uninstall Plugin...", self)
        plugin_uninstall_act.triggered.connect(self.uninstall_plugin)
        ...
        plugin_menu.addAction(plugin_install_act)
        plugin_menu.addAction(plugin_reload_act)
        plugin_menu.addAction(plugin_uninstall_act)

        self._actions = {
            "new": new_act, "open": open_act, "save": save_act, "save_as": save_as_act,
            "close": close_act, "exit": exit_act, "undo": undo_act, "redo": redo_act, "theme": theme_act
        }
    ...
    def save_file(self):
        ...
        try:
            ...
            editor.set_saved_state()
            self.refresh_tab_title_for_editor(editor)
            self.hooks.on_save(str(editor.file_path))
        except Exception as e:
    ...
    def save_file_as(self):
        ...
        try:
            ...
            self.tabs.setTabText(idx, editor.file_path.name)
            self.tabs.setTabToolTip(idx, str(editor.file_path))
            self.hooks.on_save(str(editor.file_path))
        except Exception as e:
    ...
    def install_plugin(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Install failed", f"{e}")
            return
    ...
    def uninstall_plugin(self):
        ...
        if not names:
            QMessageBox.information(self, "No plugins", "No installed plugins to remove.")
            return
        name, ok = QInputDialog.getItem(self, "Uninstall Plugin", "Select plugin to uninstall:", names, 0, False)
        if not ok or not name:
            return
        try:
            self.plugin_manager.uninstall_plugin(name)
            QMessageBox.information(self, "Uninstalled", f"{name} removed.")
        except Exception as e:
    ...
    def update_status(self):
        ...
        self.status_left.setText(f"{modified}{path}")
        self.status_right.setText(f"Line {cursor.blockNumber()+1}, Col {cursor.columnNumber()+1} — {lines}L • {chars}ch")
    ...
    def _apply_dark_theme(self):
        self.setStyleSheet("""
            ...
            QPlainTextEdit{background:#1b1b1b;color:#d4d4d4; selection-background-color:#264f78; padding:6px;}
            QTabBar::tab{background:#2d2d2d;padding:8px 12px;margin-right:2px;color:#d4d4d4;}
            QTabBar::tab:selected{background:#1f1f1f;border-bottom:2px solid #007acc;}
    ...
    def _apply_light_theme(self):
        self.setStyleSheet("""
            QMainWindow{background:#f0f0f0;color:#2b2b2b;}
            QPlainTextEdit{background:#ffffff;color:#2b2b2b; selection-background-color:#cce0ff; padding:6px;}
            QTabBar::tab{background:#e8e8e8;padding:8px 12px;margin-right:2px;color:#2b2b2b;}
            QTabBar::tab:selected{background:#ffffff;border-bottom:2px solid #007acc;}
            QStatusBar{background:#e0e0e0;color:#2b2b2b;}
            QMenuBar{background:#e8e8e8;color:#2b2b2b;}
        """)
